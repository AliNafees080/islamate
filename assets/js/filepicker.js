...
filepicker.extend("comm", function() {
    ...
    var g = function() {
        if (f) return;
        else f = true; if (window.addEventListener) window.addEventListener("message", e, false);
        else if (window.attachEvent) window.attachEvent("onmessage", e);
        else throw new a.FilepickerException("Unsupported browser");
    };
    var h = function() {
        if (window.removeEventListener) window.removeEventListener("message", e, false);
        else if (window.attachEvent) window.detachEvent("onmessage", e);
        else throw new a.FilepickerException("Unsupported browser"); if (!f) return;
        else f = false;
        var d = document.getElementsByName(b);
        for (var g = 0; g < d.length; g++) d[g].parentNode.removeChild(d[g]);
        try {
            delete window.frames[b];
        } catch (h) {}
        var i = document.getElementsByName(c);
        for (var j = 0; j < i.length; j++) i[j].parentNode.removeChild(i[j]);
        try {
...
filepicker.extend("dragdrop", function() {
    ...
    var c = function(c, d) {
        ...
        var m = d.extensions;
        if (!m)
            if (d.extension) m = [d.extension];
        if (a.util.typeOf(m) == 'string') m = m.replace(/ /g, "").split(',');
        var n = {
            location: d.location,
            path: d.path,
        ...
        var v = function(b) {
            if (b.length > 0) {
                ...
                if (d.maxFiles > 0 && b.length > d.maxFiles) {
                    j("TooManyFiles", "Only " + d.maxFiles + " files at a time");
                    return false;
                }
                var c;
                var e;
...
filepicker.extend("exporter", function() {
    ...
    var d = function(e, f, g, h) {
        ...
        if (f.container == 'modal')
            if (a.window.shouldForce()) f.container = 'window';
            else if ((a.browser.isIOS() || a.browser.isAndroid()) && a.browser.isSafari()) f.mobile = true;
        a.window.open(f.container, f.mobile, a.urls.constructExportUrl(e, f, j), n);
        a.handlers.attach(j, c(l, m));
    };
    return {
        createExporter: d
    };
...
filepicker.extend(function() {
    ...
    var m = function(b, c, e, f) {
        ...
        } else throw new a.FilepickerException("Invalid file to export: " + b + ". Not a valid url or FPFile object. You may want to use filepicker.store() to get an FPFile to export"); if (c.suggestedFilename) c.suggestedFilename = encodeURI(c.suggestedFilename);
...
filepicker.extend("modal", function() {
    ...
    var d = function(b, c, d) {
        ...
        k.name = a.window.WINDOW_NAME;
        k.id = a.window.WINDOW_NAME;
        var l = a.window.getSize();
        var m = Math.min(l[1] - 40, 545);
        k.style.width = '100%';
        k.style.height = m - 32 + 'px';
        k.style.border = "none";
    ...
    var f = function(b) {
        ...
        d.style.padding = "10px 0 0";
        d.style.background = '#ffffff url("https://www.filepicker.io/static/img/spinner.gif") no-repeat 50% 50%';
        d.style.top = '10px';
        var e = a.window.getSize();
        if (b) {
            d.style.top = '0';
            d.style.right = '0';
            d.style.bottom = '0';
            ...
            var f = Math.min(e[1] - 40, 535);
            var g = Math.min(e[0] - 40, 800);
            d.style.top = '10px';
            d.style.bottom = 'auto';
            d.style.right = 'auto';
            d.style.height = f + 'px';
        ...
        d.style.webkitBorderRadius = '3px';
        d.style.borderRadius = '3px';
        d.style.margin = '0';
        d.style.webkitBoxShadow = '0 3px 7px rgba(0, 0, 0, 0.3)';
        d.style.boxShadow = '0 3px 7px rgba(0, 0, 0, 0.3)';
        d.style.zIndex = 10001;
        d.style.boxSizing = "content-box";
        d.style.webkitBoxSizing = "content-box";
        d.style.mozBoxSizing = "content-box";
        return d;
    };
    var g = function(a) {
...
filepicker.extend("picker", function() {
    ...
    var h = function(f, i, j, k, l, m) {
        ...
        if (f.debug) {
            ...
            if (k || f.storeLocation) dumy_callback = [dumy_data, dumy_data, dumy_data];
        ...
        var p = false;
        var q = function(a) {
            if (f.container == 'window') window.onbeforeunload = null;
            p = true;
            i(a);
        };
        var r = function(a) {
        ...
        var u;
        if (k) {
            t = a.urls.constructPickUrl(f, o, true);
            u = g(q, r, m);
        } else if (l) {
            t = a.urls.constructPickFolderUrl(f, o);
            u = d(q, r);
        } else {
            t = a.urls.constructPickUrl(f, o, false);
            u = c(q, r);
        }
        a.window.open(f.container, f.mobile, t, s);
        a.handlers.attach(o, u);
        var v = o + "-upload";
        a.handlers.attach(v, e(function() {
            a.handlers.detach(v);
        }));
    };
    return {
...
filepicker.extend("urls", function() {
    ...
    var g = b + "/api/store/";
    var h = function(b, c, e) {
        return d + "?key=" + a.apikey + "&id=" + c + "&referrer=" + window.location.hostname + "&iframe=" + (b.container != 'window') + "&version=" + a.API_VERSION + (b.services ? "&s=" + b.services.join(",") : "") + (b.container !== undefined ? "&container=" + b.container : "modal") + (e ? "&multi=" + !! e : "") + (b.mimetypes !== undefined ? "&m=" + b.mimetypes.join(",") : "") + (b.extensions !== undefined ? "&ext=" + b.extensions.join(",") : "") + (b.openTo !== undefined ? "&loc=" + b.openTo : "") + (b.language ? "&language=" + b.language : "") + (b.maxSize ? "&maxSize=" + b.maxSize : "") + (b.maxFiles ? "&maxFiles=" + b.maxFiles : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "") + (b.mobile !== undefined ? "&mobile=" + b.mobile : "") + (b.folders !== undefined ? "&folders=" + b.folders : "") + (b.storeLocation ? "&storeLocation=" + b.storeLocation : "") + (b.storePath ? "&storePath=" + b.storePath : "") + (b.storeContainer ? "&storeContainer=" + b.storeContainer : "") + (b.storeAccess ? "&storeAccess=" + b.storeAccess : "");
    };
    var i = function(b, c) {
        return f + "?key=" + a.apikey + "&id=" + c + "&referrer=" + window.location.hostname + "&iframe=" + (b.container != 'window') + "&version=" + a.API_VERSION + (b.services ? "&s=" + b.services.join(",") : "") + (b.container !== undefined ? "&container=" + b.container : "modal") + (b.openTo !== undefined ? "&loc=" + b.openTo : "") + (b.language ? "&language=" + b.language : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "") + (b.mobile !== undefined ? "&mobile=" + b.mobile : "");
    };
    var j = function(b, c, d) {
        if (b.indexOf("&") >= 0 || b.indexOf("?") >= 0) b = encodeURIComponent(b);
        return e + "?url=" + b + "&key=" + a.apikey + "&id=" + d + "&referrer=" + window.location.hostname + "&iframe=" + (c.container != 'window') + "&version=" + a.API_VERSION + (c.services ? "&s=" + c.services.join(",") : "") + (c.container !== undefined ? "&container=" + c.container : "modal") + (c.openTo !== undefined ? "&loc=" + c.openTo : "") + (c.mimetype !== undefined ? "&m=" + c.mimetype : "") + (c.extension !== undefined ? "&ext=" + c.extension : "") + (c.language ? "&language=" + c.language : "") + (c.mobile !== undefined ? "&mobile=" + c.mobile : "") + (c.suggestedFilename !== undefined ? "&defaultSaveasName=" + c.suggestedFilename : "") + (c.signature ? "&signature=" + c.signature : "") + (c.policy ? "&policy=" + c.policy : "");
    };
    var k = function(b) {
        return g + b.location + "?key=" + a.apikey + (b.base64decode ? "&base64decode=true" : "") + (b.mimetype ? "&mimetype=" + b.mimetype : "") + (b.filename ? "&filename=" + encodeURIComponent(b.filename) : "") + (b.path ? "&path=" + b.path : "") + (b.container ? "&container=" + b.container : "") + (b.access ? "&access=" + b.access : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "");
    };
    var l = function(a, b) {
...
filepicker.extend("widgets", function() {
    ...
    var d = function(d) {
        ...
        b("language", f, "data-fp-language", d);
...
filepicker.extend('window', function() {
    ...
    var g = function() {
        var b = (a.browser.isIOS() || a.browser.isAndroid());
        if (b && a.browser.isMobileSafari()) {
            var c = false;
            var b = false;
            var d = false;
        } else {
            var c = f()[0] < 768;
            var d = a.cookies.THIRD_PARTY_COOKIES === false;
        }
        return (c || d || b);
    };
    var h = function(b, f, h, i) {
        i = i || function() {};
        if (!b) b = 'modal';
        if (b == 'modal' && g()) b = 'window';
        if (b == 'window') {
            var j = c + a.util.getId();
            window.onbeforeunload = function k() {
                return "Filepicker upload does not complete.";
            };
            var l = window.open(h, j, d);
            if (!l) {
                window.onbeforeunload = null;
                alert('Please disable your popup blocker to upload files.');
            }
            ...
            var n = f ? window.JSON.parse(f) : false;
            a.modal.generate(h, n, i);
        } else {
            var o = document.getElementById(b);
            if (!o) throw new a.FilepickerException("Container '" + b + "' not found. This should either be set to 'window','modal', or the ID of an iframe that is currently in the document.");
            o.src = h;
        }
    };
    return {
